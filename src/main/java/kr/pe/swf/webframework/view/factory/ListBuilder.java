...
public class ListBuilder {
    ...
    public String createBindGrid(int count, int index, String targetListQKey, String url, String layout, Map param){
        ...
        // 리스트가 2개 이상이고 다음 타겟 리스트가 있을때 타겟리스트 조회 이벤트 설정
        if(count > (index+1)){
            String targetId   = "LISTS" + (index+2);

            buffer.append("onSelectRow: function(rowid, status, e){").append("\r\n");
            buffer.append("\t\tvar server = new serverUtil(null, '").append(targetId).append("','").append(targetListQKey).append("','").append(StringUtils.trimStr(url)).append("');").append("\r\n");
            buffer.append("\t\tserver.setParam('rowKey', rowid);").append("\r\n");
            buffer.append("\t\tserver.loadGrid();").append("\r\n");
            buffer.append("\t},").append("\r\n");
        }

        // 사용자 이벤트
        List<EventEntry> eventEntryList = this.listsFactory.getEventEntries();
        ...
        if(this.listsFactory.isFirstLoad()){
            ...
            buffer.append("url:'").append((StringUtils.isEmpty(this.listsFactory.getUrl()) ? "commonGridSelectList.do"
                    : this.listsFactory.getUrl())).append("',\r\n");
